...
if ($id) {
  ...
  $new_poll_link = route('@new_thread', $id, array('poll' => 1));
  $new_thread_link = route('@new_thread', $id);
  $newthreadbar =
...
elseif($user)
  $threads = $sql->query("SELECT t.*, minpower, f.id as forumid, "
    ."'".mysql_real_escape_string($user1['name'])."' AS name1, {$user1['sex']} AS sex1, {$user1['powerlevel']} AS pwr1, '"
    .mysql_real_escape_string($user1['aka'])."' as aka1, {$user1['birthday']} as bd1, "
