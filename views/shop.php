<?php
	...
	if ($_GET['action'] == "buy" && $_GET['id'] == 286) {
		return header("Location: {$GLOBALS['jul_views_path']}/shoph.php");
	}
  ...
  if(!$log){
	...
	case '':
	  ...
	  while($shop=mysql_fetch_array($shops))
	    $shoplist.="
		<tr>
		$tccell1><a href={$GLOBALS['jul_views_path']}/shop.php?action=items&cat=$shop[id]#status>$shop[name]</a></td>
	...
	case 'items':
	  ...
	  $eqitem=mysql_fetch_array(mysql_query("SELECT * FROM items WHERE id=$eq[e]"));
		...
		$tblstart
		  $tccell1><a href={$GLOBALS['jul_views_path']}/shop.php>Return to shop list</a>
		$tblend
		<a name=status>
		<table><td width=256>
		 <img src={$GLOBALS['jul_views_path']}/status.php?u=$loguserid>
	  ...
	  while($item=mysql_fetch_array($items)){
		...
		$comm="width=80 colspan=2><a href={$GLOBALS['jul_views_path']}/shop.php?action=sell&cat=$cat>Sell</a>";
	    }elseif($item[id] && $item[coins]<=$GP && $item[gcoins] <= $user['gcoins']){
		$comm="width=30><a href={$GLOBALS['jul_views_path']}/shop.php?action=buy&id=$item[id]>Buy</a></td>$tccell1 width=50>$preview";
	    }elseif(!$eq[e] && !$item[id]){
	...
	case 'buy':
	  ...
	  if($item && $item[coins]<=$GP && $item['gcoins'] <= $user['gcoins']){
		...
		$tblstart
		  ...
		  ".redirect("{$GLOBALS['jul_views_path']}/shop.php",'return to the shop',0)."
		$tblend
	    ";
	  }
	break;
